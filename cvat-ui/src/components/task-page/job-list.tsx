...
import { Row, Col, Icon, Table, Button, Tooltip } from "antd";
...
function JobListComponent(props: Props): JSX.Element {
  const { taskInstance, registeredUsers, onJobUpdate, user } = props;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (jobInstance: any): JSX.Element => {
        const assignee = jobInstance.assignee
          ? jobInstance.assignee.username
          : null;
        ...
        if (!user.isSuperuser) {
          if (assignee) {
            ...
            return (
              <Button
                onClick={() => {
                  jobInstance.assignee = user;
                  onJobUpdate(jobInstance);
                }}
              >
                <Text>Get the Job!</Text>
              </Button>
            );
          }
        } else {
          return (
            <UserSelector
              ...
              onChange={(value: string): void => {
                let [userInstance] = [...registeredUsers].filter(
  ...
  const data = jobs.reduce((acc: any[], job: any) => {
    ...
    acc.push({
      ...
      duration: `${moment
        .duration(moment(moment.now()).diff(created))
        .humanize()}`,
      assignee: job
    });
            ...
            <Button
              ...
              onClick={(): void => {
                ...
                for (const job of taskInstance.jobs) {
                  serialized += `Job #${job.id}`.padEnd(
                    `${latestJob.id}`.length + 6,
                    " "
                  );
